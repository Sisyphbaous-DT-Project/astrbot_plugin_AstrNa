...
- 控制台只管理 20 个功能主开关，开关状态来自真实配置，并与原插件配置页双向同步；保存失败会回滚并提示。
- 子配置（模型选择、白名单、允许列表等）在控制台中只读展示“已配置/未配置”或数量摘要，需前往原插件配置页调整。
- GitHub Token 等敏感内容只显示“已配置/未配置”，完整 UMO、群号不会展示。
