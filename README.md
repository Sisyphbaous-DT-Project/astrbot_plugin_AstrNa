...
AstrNa 主要面向 AstrBot 当前 4.x 版本。当前正式版 `1.6.3` 已在 AstrBot `4.28.1` 源码环境中完成回归验证。
...
默认环境全量测试结果为 `896 passed, 15 skipped`；绑定 AstrBot `4.28.1` 源码为 `928 passed`；绑定 `4.28.0` 为 `910 passed, 1 skipped`（新版图片准备流程的专属测试按能力探测跳过）。绑定源码环境仍有已知第三方及同步配置读取的弃用警告。

本轮适配了官方默认 `astrbot/<当前版本>` UA 的精确匹配，以及引用转述新增的 `image_ref` 参数与跨模块调用入口。回复历史和图像转述在完整 Runtime 中按同一顺序安装、恢复；失败引用图通过本轮来源记录定位，回取后在转述前重新完成原生图片准备。回归覆盖整条消息回取时的部分失败去重、URL 下载失败、GIF/BMP/大图处理、同路径文件修复、并发请求隔离、停止与卸载、Dashboard 开关首条请求和 UA 独立测试。当前消息转述的自定义提示词保持可用。此前版本的兼容结果见历史发布说明。
