...
<p align="center">
  <img src="logo.png" alt="AstrNa 图标" width="256">
</p>
...
| 供应商会话请求头 | 关闭 | 给所有 LLM 供应商请求附加按会话稳定的 `x-opencode-session` 头与真实 User-Agent，供支持它的上游优化路由和提示词缓存。 |
...
| 解锁群聊并发回复（实验性） | 关闭 | ⚠️ 实验性。同群不同群友的 LLM 后台并发，每轮消息按群连续发送；历史只合并可信并发新增轮次，不复活已截断旧历史。 |
...
- 真实昵称：补充或替换群昵称。
- 补充群成员身份：通过 NapCat / aiocqhttp 查询当前发言人的群身份（群主、管理员、普通成员）、群等级和专属头衔，并注入本群群主与全部管理员的昵称和 QQ 号（名单按群缓存 24 小时，刷新前群主/管理员变更不会立即反映）。
...
AstrBot 主流程已内置引用图片收集。在提供独立图片准备入口的版本中，AstrNa 会记录当前 Reply 嵌入图片的来源与下载路径；发现下载失败、本地文件失效或图片内容不可解码时，通过 NapCat / aiocqhttp 的 `get_msg` / `get_image` / `get_file` 等 OneBot 接口尝试恢复。回取整条消息时只选择确认失败的图片，保留成功图片；恢复结果会在原生图像转述前重新完成格式转换、动画处理和缩放，再由纯文本主模型接收转述文字，或由多模态主模型接收图片。Dashboard 保存本开关后，对下一条新请求立即生效。

第三方自建请求仍可在每轮 LLM 请求前补齐当前引用图片，并在补图成功时注入一句“当前消息引用了 N 张图片，已作为本轮视觉输入提供。”，该提示不写入会话历史。已进入原生转述的请求不会在此阶段重新追加原图。
...
- 准备阶段的恢复只针对上游已尝试收集的 Reply 嵌入图片；回取结果无法通过图片标识或相同图片数量下的原顺序可靠匹配时，不猜测替代对象。被上游数量上限排除的图片不回填；仅回复 ID 路径远程取得的图片失败时，不再次回取。
- 旧版没有独立图片准备入口时保留请求阶段兜底，只按引用字符串去重，不能保证把 URL 与本地副本合并为同一图片。
- 可能增加本轮图片 token，默认关闭。
...
- 白名单 UMO 可逐项添加多个，也支持批量导入；可用它放行需要长回复的群聊或私聊，命中后不会关闭流式，也不会限制输出。
...
- 只处理 Bot 最终普通文本回复。
- skills_like 模式下，工具参数二次询问退回的普通最终回答同样会在首次发送前经过限制，发送内容与历史记录一致采用清洗结果。
...
- 只选择互不依赖、主要返回数据、不会直接操纵聊天或共享状态的工具；后一个调用需要前一个结果时必须拆成两批。
- `astrna_parallel_tool_use` 自身、`send_message_to_user`、Handoff 和后台任务永远禁止加入并发名单。
- 多个插件、MCP 或 AstrBot 内置工具共用同一个工具名时，该名称会标记为歧义项并拒绝并发，避免管理员选择的工具与运行时实际对象不一致。
- 完全相同的工具名与参数不能在同一批重复提交，避免有副作用的工具被执行两次。
- 内部使用 `asyncio.gather` 真正并行执行，并按提交顺序返回逐项 `recipient_name / ok / result`；单项异常和超时不会抹掉其他项的结果。
- 每个子工具继续走 AstrBot 原执行器、参数过滤、权限复核和 `on_tool_start / on_tool_end` 钩子。
- 单项超时按该工具从开始到结束的总时长计算（AstrBot 原生逐段等待是每等下一段结果单独计时，批量模式更严格）；超时上限沿用 AstrBot 的 `tool_call_timeout`。
- `/stop` 和外层 Agent 任务取消都会传播到并发批次：整批子调用会被一并取消并回收，不会留下继续运行的子工具；但 AstrBot 原生单工具在外层取消时本身可能残留后台任务，那是上游行为，与本功能无关。
- 多段文本会全部聚合；图片和文本/图片资源会保留在外层 `CallToolResult` 中，继续交给 AstrBot 原生图片缓存与大结果落盘流程处理。空结果或只有不支持内容时会明确标记失败，不会伪装成功。

### 供应商会话请求头

这是 `1.6.1` 新增的功能，默认关闭。开启后，AstrNa 会给所有 LLM 供应商（OpenAI / Anthropic / Gemini 及其兼容源）发出的每个请求附加 `x-opencode-session` 会话头，并把 SDK 出厂的笼统 User-Agent（如 `AsyncOpenAI/Python`）替换为真实的 `AstrBot/<版本> AstrNa/<版本>` 标识。

会话头按会话稳定：每个群聊、私聊使用各自的会话标识，支持该头的上游可据此优化路由和提示词缓存；是否给每个会话分配独占缓存、是否每次命中由上游决定。会话标识是会话 UMO 的 sha256 摘要（只取前 32 位），**不包含**原始账号、群号或会话原文，可以放心发给上游。opencode go 等上游要求请求明确标识自身并携带 `x-opencode-session`，否则直接返回 400；开启本功能可以补上这两个头，但鉴权、额度、模型可用性、代理改写等仍由上游和你的供应商配置决定。

两个子配置（原生配置页中跟随主开关展开，也可在 AstrNa Dashboard → 供应商会话请求头 → 功能设置中修改）：

- **替换供应商默认 User-Agent**（默认开启）：只替换以 SDK 出厂前缀开头的默认值，以及与当前宿主版本完全一致的官方默认 `astrbot/<版本>`；普通手填的 User-Agent 不会被覆盖，以默认值为基础追加过自定义内容的值（如 `astrbot/<版本> MyGateway/1.0`）也视为手填保留。
- **额外会话头名**（默认留空）：除 `x-opencode-session` 外，再用同一个会话 id 多写一个自定义头名（如 `x-session-id`），供其他上游识别；仅允许字母、数字和连字符，最长 64 字符，不能覆盖保留头。

注意边界：不传会话 id 的辅助调用（如上下文压缩、报错分析）统一使用固定的 `astrna-aux` 标识，只用于携带合规会话头，不做按用户会话的缓存隔离；Gemini 供应商轮换 API key 重建连接的当次重试可能来不及挂章，下一次请求即恢复。

## 兼容性

AstrNa 主要面向 AstrBot 当前 4.x 版本。当前正式版 `1.6.4` 已在 AstrBot `4.28.1` 源码环境中完成回归验证。

部分能力依赖平台：

- 群成员身份、群等级、专属头衔、群主/管理员名单、生日查询依赖 NapCat / aiocqhttp 可查询到对应信息。
...
- LLM 并发工具调用依赖 AstrBot 的真实请求 ToolSet、权限包装、Runner 钩子和原生工具执行器，同步（`4.27.2`）与异步（`4.27.3` 起）权限检查接口均已适配；允许名单不扩大当前会话权限范围，其他版本若更改这些内部入口会自动拒绝安装或拒绝执行。
- 供应商会话请求头通过包装各供应商实际定义 `text_chat` / `text_chat_stream` 的类、并向其 httpx 客户端追加请求钩子实现，覆盖主会话、备用供应商、流式与第三方插件直接调用；能否生效取决于 AstrNa 是否能找到该供应商实际使用的 httpx 客户端，完全不走 httpx 或额外自建未纳管客户端的请求不会获得会话头。
- 引用图片视觉输入优化依赖 AstrBot 自带引用图片解析能力。提供独立图片准备入口时，AstrNa 可恢复下载或准备失败的 Reply 嵌入图片，支持 NapCat / aiocqhttp 的 `bot.call_action`，并在转述前重新准备恢复图；旧版保留请求阶段兜底。仅回复 ID 的远程图片不二次回取，不回填超出上游数量限制的图片，可能增加本轮图片 token。
...
- 输出字数限制会包装 AstrBot 普通 LLM runner，只限制最终纯文本回复；skills_like 模式下工具参数二次询问退回的普通最终回答也会经过限制；开启后非白名单会话会关闭本轮流式输出，Live Mode 会保留流式并跳过限制。
...
本轮（`1.6.4`）为针对性修复，验证以相关套件为主：默认环境限制器与关联回归 `99 passed, 2 skipped`；绑定 AstrBot `4.28.1` 的联合套件 `256 passed`（同进程覆盖输出字数限制、Runtime 生命周期、群聊并发与工具相关模块）；真实发送链 `/stop`、第三方包装共存、卸载重装、旧请求隔离、清洗失败兜底等独立复现全部通过；`ruff` 与 `git diff --check` 通过。本轮未跑全量套件；最近一次全量口径（`1.6.3`）为默认环境 `906 passed, 15 skipped`、绑定 `4.28.1` `938 passed`。

本轮修复输出字数限制在 skills_like 模式下的漏检：工具参数二次询问退回的普通最终回答此前直接发送并写入历史、不经过清洗，现在会在首次发送前经过现有限制流程，发送与历史一致采用清洗结果；工具调用、白名单、Live Mode、`/stop` 与清洗失败兜底行为不变。回归覆盖 completion_text / 纯文本 result_chain 两种形态、工具调用直通、空二次询问后修复重询再退回、共享链重排的在途清洗和停止/卸载迟到隔离。此前版本的兼容结果见历史发布说明。
