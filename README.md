...
AstrNa 不会整组删除工具消息：`assistant.tool_calls`、调用参数、每个 `tool_call_id`、并行工具的数量与顺序、最终 assistant 回答都会保留，避免破坏 OpenAI compatible 等供应商要求的工具调用配对。孤立工具结果、并行结果不完整、尚未被后续 assistant 消费的尾部工具链会保留原文，不会冒险改写。
