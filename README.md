...
- **替换供应商默认 User-Agent**（默认开启）：只替换以 SDK 出厂前缀开头的默认值；普通手填的 User-Agent 不会被覆盖，但以 SDK 前缀开头的手填值会被视为默认值替换。
...
AstrNa 主要面向 AstrBot 当前 4.x 版本。当前正式版 `1.6.1` 已在 AstrBot `4.28.0-beta.1` 源码环境中完成回归验证。
