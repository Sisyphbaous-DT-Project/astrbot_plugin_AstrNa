...
def test_config_schema_is_valid_json_and_has_expected_defaults():
    ...
    assert "max_turns" in schema["optimize_group_chat_context"]["hint"]
    assert "trim_turns" in schema["optimize_group_chat_context"]["hint"]
...
def test_changelog_contains_release_notes():
    ...
    assert "## 1.5.0.beta1" in changelog
    assert "## 1.5.0.beta2" in changelog
    ...
    assert "## 1.6.2" in changelog
    assert "## 1.6.3" in changelog
...
def test_readme_current_version_matches_metadata():
    ...
    assert "当前已测试兼容 AstrBot 版本：`4.28.1`" in readme
